<?php
	...
	<script>
	    ...
	    $("#apa").attr("readonly","readonly");
	    $("#depto").attr("readonly","readonly");
	    var lista= document.getElementById('na');
	    lista.options[(<?=$datosID[5]?>)+1].selected= true;
	</script>
	<?
    }
?>
...
<div id="encabezado" style="height: 305px; width: 810; float: left;">
    ...
    <div id="datosPrincipales" style="margin-left: 100px; margin-top: 10px;">
	    <table class="datprin">
	        ...
	        <tr>
		    <td>Nombre:</td>
		    <td><input type="text" name="nombre" value="<?=$datosID[3];?>" id="nombre" onchange="$('#usuario').attr('value','');" onblur="esconde('error');" onkeyup="this.value = this.value.replace (/[^aA-zZ ]/,'');" onpaste="return false"></td>
	        </tr>
	        <tr>
		    <td>Apellido Paterno:</td>
		    <td><input type="text" name="apa" value="<?=$datosID[4];?>" id="apa" onchange="$('#usuario').attr('value','');" onblur="esconde('error');" onkeyup="this.value = this.value.replace (/[^aA-zZ ]/,'');" onpaste="return false"></td>
		</tr>
		<tr>
		    <td>Sexo:</td>
		    <td>
		...
		<tr>
		    <td>Usuario:</td>
		    <td><input type="text" name="usuario" value="<?=$datosID[1];?>" id="usuario" readonly="" onfocus="crearusuario();" onblur="esconde('error');"></td>
		...
		<tr>
		    <td>Departamento:</td>
		    <td><input type="tex" name="depto" value="<?=$datosID[20];?>" id="depto" onblur="esconde('error');"></td>
		</tr>
		<?
		 if($opcion==1 && $id_usuario){
		    ?>
			...
			<td colspan="2" align="center"><div id="resetp"><input type="button" value="Resetear Password" name="reset" id="reset" onclick="reset();"></div></td>
		    </tr>
		    <?
		 }
		?>
